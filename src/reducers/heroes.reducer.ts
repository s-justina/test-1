    ...
    switch(action.type){
        ...
        case HeroesActionName.ADD_CREATED_HERO:
            ...
            return {
                ...
                heroesListToDisplay: newHeroesList.slice(0, state.perPage * state.currentPage)
            }
        case HeroesActionName.REMOVE_DELETED_HERO:
            const newHeroesListAfterHeroRemoval = state.heroesList.filter((hero) => hero.id !== action.payload.id);
            return {
                ...state,
                heroesList: newHeroesListAfterHeroRemoval,
                heroesListToDisplay: newHeroesListAfterHeroRemoval.slice(0, state.perPage * state.currentPage)
            }
        case HeroesActionName.DISPLAY_MORE_HEROES:
            ...
            return {
                ...
                heroesListToDisplay: state.heroesList.slice(0, state.perPage * newCurrentPage)
            }
        case HeroesActionName.RESET_HEROES_LIST_TO_DISPLAY:
            return {
                ...state,
                currentPage: heroesInitialState.currentPage,
                heroesListToDisplay: state.heroesList.slice(0, heroesInitialState.currentPage)
            }
        default:
            return state
    }
